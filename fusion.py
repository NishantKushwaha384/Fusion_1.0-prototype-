...
def naturalize_answer(facts, extended=False):
    """
    Turns raw factual fragments into a smooth, human-like answer
    without adding fluff or hallucination.
    """
    ...
    # If only one fact → return clean sentence
    if len(facts) == 1:
        return facts[0] + "."

    # Light connectors (non-AI sounding)
    connectors = [
        "Additionally,",
        "Also,",
        "In addition,",
    ...
    for i, fact in enumerate(facts[1:]):
        if extended:
            ...
            # Concise mode = tighter flow, minimal connectors
            sentence += f" {fact}."
...
def build_natural_answer(facts, extended=False):
    ...
    # More precise definition detection
    definition, others = [], []
    for f in cleaned:
    ...
    # Safe merge: only merge if combined length is still readable
    merged = [ordered[0]]
    for f in ordered[1:]:
        prev = merged[-1]
        combined_len = len(prev.split()) + len(f.split())
        if combined_len <= 18:  # merge only if result stays concise
            merged[-1] = prev + "; " + f  # semicolon is safer than space
        else:
            merged.append(f)
...
def _step_synthesis(
    ...
    ──────────────────────────────────────────────────────────────────
    Unlike fact extraction (which breaks answers into fragments),
    step synthesis PRESERVES ORDER and STRUCTURE. Ideal for recipes,
    tutorials, and how-to guides.

    Strategy:
    1. Rank answers by confidence
    2. Use highest-confidence answer as the primary step sequence
    3. Extract additional tips/insights from secondary models
    4. Merge them into numbered steps with supporting details
    5. Include prerequisites and estimated time if available
    """
    ...
    # ── EXTRACT STEPS FROM PRIMARY ANSWER ───────────────
    # Try to identify numbered/bulleted steps
    primary_steps = _extract_steps(best_answer)

    if not primary_steps:
        # Fallback: use the best answer as-is if no structure detected
        final_answer = best_answer
        fusion_notes = f"No numbered steps detected. Using highest-confidence answer (conf={best_conf:.2f})"
    else:
        # ── EXTRACT INSIGHTS FROM SECONDARY MODELS ──────
        secondary_insights = []
        for i, ans in enumerate(sorted_answers[1:], 1):
            insights = _extract_procedural_insights(ans, primary_steps)
            if insights:
                secondary_insights.extend(insights)

        # ── MERGE PRIMARY STEPS WITH SECONDARY INSIGHTS ──
...
def _extract_steps(text: str) -> List[str]:
    ...
    # Pattern 1: Numbered with period/colon/parenthesis (1. 2) or (1: 2:)
    numbered = re.findall(
        r'^\s*\d+[\.\:\)]\s+(.+?)(?=^\s*\d+[\.\:\)]|\Z)',
        text,
...
def _extract_procedural_insights(text: str, primary_steps: List[str]) -> List[str]:
    """
    Extract useful tips, warnings, or alternative approaches from text.
    Looks for phrases like:
      - "Tip:", "Tip -", "Note:", "Pro tip", "Warning:"
      - "Alternative:", "You can also..."
      - Sentences that reference multiple steps or offer variations
    """
    insights = []

    # Extract tip/note/warning blocks
    tip_patterns = [
        r'(?:tip|pro tip)[\:\-]?\s*(.+?)(?=\n|tip|note|warning|\Z)',
        r'(?:note|important)[\:\-]?\s*(.+?)(?=\n|tip|note|warning|\Z)',
        r'(?:warning|caution)[\:\-]?\s*(.+?)(?=\n|tip|note|warning|\Z)',
...
def _merge_steps_with_insights(
    ...
    Output is clearly structured with:
      1. Prerequisites (if any)
      2. Numbered steps
      3. Tips/Notes
    """

    # Start with best answer context (may contain prerequisites)
    output_lines = []

    # Check for prerequisite-like statements in the best answer
    prereq_patterns = [
    ...
    # Add secondary insights as a tips section
    if secondary_insights:
        output_lines.append("\nTips & Insights:\n")
        for i, insight in enumerate(secondary_insights[:3], 1):  # Limit to top 3
